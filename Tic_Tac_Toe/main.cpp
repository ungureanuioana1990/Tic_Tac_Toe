...
void runGame()
{
	...
	if (winner == "C")
	{
		...
		cout << "The winner is " << winner << endl; // print's x or 0
	}
...
void getUserInput(bool xTurn, string gameBoard[ROWS][COLS])
{
	...
	while (keepAsking)
	{
		...
		if (row >= 0 && col >= 0 && row <= 2 && col <= 2)
		{
			...
			//but it STILL could be occupied by an X or 0 already...
...
string getWinner(string gameBoard[ROWS][COLS])
{
	...
	for (int i = 0; i < ROWS; i++)
	{
		...
		//upper-left to buttom right diagonal
	...
	bool isBoardFull(string gameBoard[ROWS][COLS])
	{
		...
		for (int i = 0; i < ROWS; i++)
		{
	     ...
	     return countFill == 9; // all 9 cells are full, them board is full
       }